...
**默认视角 = 完整基地，绝不裁切**：画布 `overflow:hidden` 且等于网格矩形，所以写死放大倍率会
把基地左右两侧切掉（曾用 1.6，底座四角越界 20%，玩家反馈"场景左右两边被切掉一块"）。
`WarmOrbitCamera.fitDefaultZoom(底座半边长, 顶高)` 在**默认角度**下把底座（13/2+0.25=6.75）与建筑顶
（2.6）投影一遍，迭代三次反推「刚好不裁切」的倍率（本机 13×13 下 ≈1.246，四角 NDC 0.98）。
⌂ 回正与开局都用它；放大是玩家主动行为（那时裁切属预期），地图扩大/窗口变化会自动重算。
验收：`check-base-3d.cjs`「默认视角基地完整不裁切」+「边长占用 ≥ 85%」两条用例。
