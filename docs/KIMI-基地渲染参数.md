...
**放大上限 = 基地刚好铺满可用宽度**：`fitMaxZoom(底座半边长, 顶高, 可用宽度)` 同法迭代，
可用宽度 = `min(窗口宽, 游戏画布宽)`（宽屏桌面上游戏画布只占中间一条，按窗口宽算会把 UI 全盖住）。
上限的意义：3D 层铺满整屏后，再放大就会盖住 HUD/卡片栏——卡在这里基地既能到最大尺寸，
又不出屏、不压 UI（只溢出网格矩形）。验收：`check-base-3d.cjs`「放大到底被钳位在…」+
「宽度 ≈ 可用宽度（不出屏）」+「不压卡片栏」三条用例。
