...
(async()=>{const b=await chromium.launch();try{const p=await b.newPage({viewport:{width:1000,height:1100}}),errors=[];p.on('pageerror',e=>errors.push(e.message));
 const manifest=JSON.parse(fs.readFileSync('assets/models/blender-samples/keys-manifest.json'));assert.equal(manifest.length,5);
 ...
 await p.goto('http://127.0.0.1:58922/models/blender-samples/voxel.html?chain=keys');await p.waitForFunction(()=>window.voxelReady);assert.equal(await p.locator('article').count(),5);for(const m of await p.evaluate(()=>voxelChecks)){assert(Math.max(...m.size)<.84);assert.equal(m.bottom,0);}
 await p.screenshot({path:'assets/models/blender-samples/keys-preview.png',fullPage:true});
 await p.setViewportSize({width:390,height:844});assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await p.setViewportSize({width:1000,height:1100});await p.goto('http://127.0.0.1:58922/merge/?voxel=1&keys=1');await p.waitForFunction(()=>window.mergeDemo);
 for(let level=0;level<5;level++){
   ...
   assert.equal(await p.evaluate(i=>mergeDemo.items[i]?.id,level*2+1),level===4?30023:30020+level,`level ${level+1}`);
 }
 assert.equal(await p.evaluate(()=>mergeDemo.state.merges),4);await p.locator('#reset').click();await p.screenshot({path:'assets/models/blender-samples/keys-board.png'});assert.deepEqual(errors,[]);console.log('PASS: 5 distinct GLBs, budgets, bounds, gallery/mobile, 4 merge transitions and max-level refusal.');
