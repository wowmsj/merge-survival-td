...
# ================= 1 基础规则 =================
w('## 1. 基础规则\n')
w('- **物品层**：与基地 13×13 网格一一对应（`types.ts` IGrid；独立棋盘已废弃，基地即合成场）。可承载格需已认领、无建筑、无地形（`GameState.ts` canHostItem）。物品落点：发射器产出优先落周围九宫可承载空格，九宫满则落全盘首个可承载空格；无处可落则点击失败。')
...
check('道具总数 454', len(PROPS) == 454)  # 451（合成链重构 2026-08-06）后陆续新增
