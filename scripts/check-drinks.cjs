...
(async()=>{const b=await chromium.launch();try{const p=await b.newPage({viewport:{width:1000,height:1100}}),errors=[];p.on('pageerror',e=>errors.push(e.message));
 const manifest=JSON.parse(fs.readFileSync('assets/models/blender-samples/drinks-manifest.json'));assert.equal(manifest.length,10);
 const hashes=new Set();for(const m of manifest){const file=fs.readFileSync(`assets/models/blender-samples/voxel_32_${m.id}.glb`);assert.equal(file.readUInt32LE(0),0x46546c67);assert.equal(file.readUInt32LE(8),file.length);hashes.add(require('node:crypto').createHash('sha256').update(file).digest('hex'));assert(m.triangles<12000);}assert.equal(hashes.size,10);
 await p.goto('http://127.0.0.1:58922/models/blender-samples/voxel.html?chain=drinks');await p.waitForFunction(()=>window.voxelReady);assert.equal(await p.locator('article').count(),10);for(const m of await p.evaluate(()=>voxelChecks)){assert(Math.max(...m.size)<.84);assert.equal(m.bottom,0);}
 await p.screenshot({path:'assets/models/blender-samples/drinks-preview.png',fullPage:true});
 await p.setViewportSize({width:390,height:844});assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await p.setViewportSize({width:1000,height:1100});await p.goto('http://127.0.0.1:58922/merge/?voxel=1&drinks=1');await p.waitForFunction(()=>window.mergeDemo);
 for(let level=0;level<10;level++){
   ...
   assert.equal(await p.evaluate(i=>mergeDemo.items[i]?.id,level*2+1),level===9?20020:20012+level,`level ${level+1}`);
 }
 assert.equal(await p.evaluate(()=>mergeDemo.state.merges),9);await p.locator('#reset').click();await p.screenshot({path:'assets/models/blender-samples/drinks-board.png'});assert.deepEqual(errors,[]);console.log('PASS: 10 distinct GLBs, budgets, bounds, gallery/mobile, 9 merge transitions and max-level refusal.');
