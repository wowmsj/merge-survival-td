...
(async()=>{const b=await chromium.launch();try{const p=await b.newPage({viewport:{width:1000,height:1100}}),errors=[];p.on('pageerror',e=>errors.push(e.message));
 const manifest=JSON.parse(fs.readFileSync('assets/models/blender-samples/voxel-chain-manifest.json'));assert.equal(manifest.length,11);
 ...
 await p.goto('http://127.0.0.1:58922/models/blender-samples/voxel.html?chain=1');await p.waitForFunction(()=>window.voxelReady);assert.equal(await p.locator('article').count(),11);for(const m of await p.evaluate(()=>voxelChecks)){assert(Math.max(...m.size)<.84);assert.equal(m.bottom,0);}
 await p.setViewportSize({width:390,height:844});assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await p.setViewportSize({width:1000,height:1100});await p.goto('http://127.0.0.1:58922/merge/?voxel=1');await p.waitForFunction(()=>window.mergeDemo);
 for(let level=0;level<11;level++){
   ...
   assert.equal(await p.evaluate(i=>mergeDemo.items[i]?.id,level*2+1),level===10?10011:10002+level,`level ${level+1}`);
 }
 assert.equal(await p.evaluate(()=>mergeDemo.state.merges),10);await p.locator('#reset').click();await p.screenshot({path:'assets/models/blender-samples/toolbox-board-complete.png'});assert.deepEqual(errors,[]);console.log('PASS: 11 distinct GLBs, budgets, bounds, gallery/mobile, 10 merge transitions and max-level refusal.');
