...
export const enRuntimeUi: Record<string, string> = { ...enCommonUi,
  ...
  'toast.mergeNeedsCore': 'Requires "{core}" to merge this type of item.',
  ...
  'toast.coreResonance': 'Merge Core resonance: output upgraded!',
  'toast.coreGifted': "Grandma's Merge Core Base has been delivered.",
  'toast.coreMigrated': 'Merge Core merged into the Base Core (Lv.{level}).',
  'toast.nightStarts': 'Night falls. Current energy is retained.', 'toast.waveIncoming': 'Wave {wave}/{total} incoming!', 'toast.daybreakLoot': 'Daybreak! Loot: {loot}', 'toast.taskRelief': 'Order stuck? Xiaoman dug out some spare supplies: {item} ×{num}',
  ...
  'toast.buildingNotBuildable': 'This building cannot be placed.', 'toast.buildingLocked': 'Locked: merge {blueprint} to unlock it.', 'toast.outOfBase': 'Outside the base.', 'toast.cellOccupied': 'This cell already has a building.', 'toast.expandTerritory': 'Expand to this cell first.', 'toast.killCorridor': 'Keep an open path for enemies.', 'toast.onlyTowers': 'Only towers can be built now.', 'toast.outerCityOnly': 'Towers go in the outer ring only (the inner 9×9 is for merging).', 'toast.corridorSealed': 'Heads up: this enemy route is now sealed. Zombies will break walls or stomp blocking pieces instead.', 'toast.itemCrushed': '{item} was stomped by a zombie. Merge next to it to dig it open again.', 'toast.notEnoughCoinsBuild': 'Not enough Coins: {coins} needed (you have {have}). Sell materials on the black market.',
  'terrain.grass': 'Overgrown Grass', 'terrain.rubble': 'Rubble Pile', 'terrain.shack': 'Ruined Shack', 'terrain.woods': 'Woods', 'terrain.pond': 'Pond',
  'terrain.clearTitle': 'Clear Terrain', 'terrain.clearAction': 'Clear ({coins} Coins)', 'terrain.hint.grass': 'An overgrown lot. Clear it to make room for construction.',
  'terrain.hint.rubble': 'Rubble of a collapsed building. Blocks zombies. Clear it to build here.',
  'terrain.hint.shack': 'A ruined shack that still blocks zombies. Clear it to build here.',
  'terrain.hint.woods': 'Dead woods. Zombies wading through are slowed. Clear it to build here.',
  'terrain.hint.pond': 'A flooded pond. Ground zombies cannot cross (flying ones can). Fill it to build here.',
  'toast.terrainBlocked': 'Clear the {terrain} first', 'toast.terrainCleared': '{terrain} cleared (-{coins} Coins)', 'toast.notEnoughCoinsClear': 'Not enough Coins: clearing needs {coins}. Sell materials on the black market.',
...
Object.assign(enRuntimeUi, {
  ...
  'action.popBubble': 'Pop ({cost})', 'action.skipCooldown': 'Skip CD', 'action.view': 'View', 'action.intro': 'About', 'action.use': 'Use', 'action.convertFuel': 'Fuel', 'action.sell': 'Sell {price}',
  'core.intro.title': 'Merge Core',
  'core.tier.0': 'Common items',
  'core.tier.1': 'Common + Steel items',
  'core.tier.2': 'Common + Steel + Hi-tech items',
  'core.baseName': 'Base Core',
  'core.card.title': '{name} Lv.{level}',
  'core.card.ready': 'Charges {times}/{max} · Aura {aura}% · Tap again to emit',
  'core.card.cooling': 'Out of charges, cooling {time} · Hold for panel',
  'core.card.panel': 'Core',
  'core.panel.title': 'Base Core',
  'core.panel.name': '{name}  Lv.{level}',
  'core.panel.stock': 'Charges',
  'core.panel.cooldown': 'Cooldown',
  'core.panel.ready': 'Ready',
  'core.panel.aura': 'Core Aura',
  'core.panel.tier': 'Merge tier',
  'core.panel.upgradeCost': 'Upgrade parts',
  'core.panel.upgrade': 'Upgrade Core',
  'core.panel.maxLevel': 'Max level',
  'core.panel.upgradeHint': 'Parts come from night raids: Mysterious Part ×1, Core Fragment ×2 (by value).',
  'core.panel.repair': 'Repair Core',
  'core.panel.body': 'Tap the core to emit base materials (1 Energy per tap; wait for the cooldown when charges run out). Core Aura upgrades all spawner output.',
  'core.productsTitle': 'Base Core Lv.{level} · Products',
  'core.productsHint': 'Upgrade the core to unlock more base materials · Tap outside to close',
  'core.upgraded': 'Base Core upgraded to Lv.{level}',
  'core.upgradeMaxed': 'Base Core is at max level',
  'core.upgradeNeedMaterial': 'Not enough core parts (need {cost} value)',
  'guide.tag': 'Guide', 'guide.merge': 'Drag matching items together to merge.', 'guide.coreSpawn': 'Tap the Base Core in the middle to emit two Toolbox Handles.', 'guide.spawn': 'Tap the glowing item to produce one.', 'guide.collect': 'Keep merging to collect task items.', 'guide.submit': 'Tap a task to submit it for Stars.',
  ...
  'guide.buildTower': 'Enough Coins. Build an Arrow Tower from the build bar below to guard the eastern breach.', 'guide.towerCost': 'The Arrow Tower costs {cost} Coins and you have {have}. Complete a task or sell materials first.', 'guide.powerEmitter': 'The Arrow Tower has no power. Tap the power-station blueprint box twice for free fragments.', 'guide.powerBlueprintMerge': 'Merge blueprint fragments into a power-station blueprint.', 'guide.unlockGenerator': 'Select the full blueprint, then tap Use to unlock the generator.',
  'guide.buildGenerator': 'Enough Coins. Build the Wind Power Station from the build bar below to bring the grid online.', 'guide.generatorCost': 'The Wind Power Station costs {cost} Coins and you have {have}. Complete a task or sell materials first.',
  ...
  'task.diamondComplete': 'Finish now: {cost} diamonds',
...
Object.assign(enRuntimeUi, {
  ...
  'base.back': 'Back', 'base.day': 'Day {day}', 'base.coreHp': 'Core: {hp}/{maxHp}', 'base.night': 'Face Night', 'base.routeOn': 'Routes: On', 'base.routeOff': 'Routes: Off', 'base.blackMarket': 'Black Market', 'base.marketStars': 'Stars: {star}', 'base.marketWallet': 'Stars: {star}  Diamonds: {diamond}', 'base.marketExchange': '1 Diamond = 100 Coins', 'base.marketExchanged': 'Exchanged for 100 Coins', 'base.marketPrice': '{star} Stars', 'base.marketComplete': 'Complete blueprint', 'base.marketBought': '{building} blueprint purchased', 'base.marketMaterials': 'Needed Materials · Buy with Stars', 'base.marketBlueprints': 'Blueprints', 'base.marketNeedQty': 'Orders need ×{num}', 'base.marketMaterialBought': 'Purchased {item}', 'base.recommendedCounter': 'Recommended Counter', 'base.recommendedCounterBody': 'Buy the complete {building} blueprint directly at the Black Market.', 'base.none': 'None', 'base.resourceGain': 'Resource building output: {gain}',
  'base.heroJoined': '{hero} joined the fortress! Deploy them from the Base Heroes tab to defend the inner zone.', 'base.noPower': 'No Power', 'base.antiAir': 'AA', 'base.buildCancel': 'Tap the building button again to cancel placement.',
  ...
  'base.buildingDesc.101': 'Ground fire only. Radar coverage enables anti-air.', 'base.buildingDesc.102': 'Heavy shell damages the target and enemies within 1 tile. Slow fire rate. Can attack air.', 'base.buildingDesc.103': 'Ignores armor and chains targets. Can attack air.', 'base.buildingDesc.104': 'Slows enemies in range. Can attack air.',
  ...
  'base.buildingDesc.301': 'Damages ground enemies that cross it. Cannot hit air.', 'base.buildingDesc.302': 'Explodes once when stepped on. Cannot hit air.', 'base.buildingDesc.303': 'Strongly slows ground enemies that cross it. Each slowed enemy costs 1 durability. Cannot hit air.',
  ...
  'base.buildingLocked': '{building} is locked: merge {blueprint} fragments to get {blueprint}.', 'base.notEnoughCoins': 'Not enough Coins: need {coins} and you have {have}. Sell materials on the black market.',
...
Object.assign(enRuntimeUi, {
  ...
  'account.hint': 'Sign in to sync your progress to the cloud. Switch devices or clear your cache without losing a thing.',
  'account.guestRow': 'Guest · Tap to sign in and back up',
  'account.email': 'Email',
  ...
  'account.signingIn': 'One moment...',
  'account.loggedInAs': 'Signed in as',
  'account.syncNow': 'Sync Now',
  'account.signOut': 'Sign Out',
  'account.bound': 'Progress linked to your account',
  'account.syncDone': 'Synced to the cloud',
  'account.syncFail': 'Sync failed. It will retry on the next save.',
  'account.signupConfirmSent': 'Account created! Check your inbox and click the verification link, then sign in.',
  'account.resetSent': 'Password reset email sent. Follow the instructions in the email.',
  'account.conflict.title': 'Save Conflict',
  'account.conflict.body': 'This account has a different save in the cloud than on this device. Choose which one to keep:',
  'account.conflict.cloud': 'Cloud Save\nDay {day} · {time}',
  'account.conflict.local': 'This Device\nDay {day} · {time}',
  'account.conflict.later': 'Decide Later',
  'account.error.noEmail': 'Please enter your email.',
  'account.error.noPassword': 'Please enter your password.',
  'account.error.invalidEmail': 'That email address does not look right.',
  'account.error.invalidCredentials': 'Incorrect email or password.',
  'account.error.emailNotConfirmed': 'This email is not verified yet. Check your inbox for the verification email.',
  'account.error.emailTaken': 'This email is already registered. Sign in instead.',
  'account.error.weakPassword': 'Password must be at least 6 characters.',
  'account.error.rateLimit': 'Too many attempts. Please try again later.',
  'account.error.generic': 'Something went wrong. Check your connection and try again.'
});
